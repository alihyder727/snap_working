...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin)
{
  ...
  //     std::string name = "s" + std::to_string(ns);
  //     SetUserOutputVariableName(3+NVAPOR+ns, name.c_str(), "passive tracer");
  //   }
  // }
}
  ...
  for (int k = ks; k <= ke; ++k)
    for (int j = js; j <= je; ++j)
      for (int i = is; i <= ie; ++i) {
        ...
        // if (NSCALARS > 0){
        //   for (int ns=0; ns<NSCALARS; ++ns){
        //     user_out_var(3+NVAPOR+ns,k,j,i) = pscalars->s(ns,k,j,i);
        //   }
        // }
      }
}
  ...
  for (int k = ks; k <= ke; ++k)
    for (int j = js; j <= je; ++j)
      for (int i = is; i <= ie; ++i) {
        ...
        temperature  = pmb->pthermo->GetTemp(w.at(k,j,i));
...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
  ...
  // std::cout << "Interpolated at 1050.0 K = " << pow(10, linearInterpolate(T_FL, xgeh4FL_powers, 1050.0)) << std::endl;
    ...
    //       for (int k = ks; k <= ke; ++k){
    //         for (int j = js; j <= je; ++j){
    //           pscalars->s(n,k,j,i) = X_CO_Eq(phydro->w(IPR,k,j,i), pthermo->GetTemp(phydro->w.at(k,j,i)), 
