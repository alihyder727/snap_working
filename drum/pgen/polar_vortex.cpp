//changes: no time loop, exp v2, vphi = vmax (positive #), coriolis not approximated, damping model--Model #2--ingersoll's equation n=2
//! \file vortex.cpp
...
// global parameters
Real phi0, phi1, lambda, alpha, vphi, vrad, vis, polarity, skewness,
omega, radius, sponge_lat, sponge_tau, max_cyclone, max_anticyclone, b;
...
Real Phib(Coordinates *pcoord, int j, int i)            //phi1 determines topography at the pole
{
...
void PolarVortexForcing(MeshBlock *pmb, const Real time, const Real dt,
    const AthenaArray<Real> &w, AthenaArray<Real> const &bcc, AthenaArray<Real> &u)
{
    ...
    for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        u(IM1,j,i) += dt*w(IDN,j,i)*phib*x1*pow(dist/lambda,alpha)/sqr(dist);
        u(IM2,j,i) += dt*w(IDN,j,i)*phib*x2*pow(dist/lambda,alpha)/sqr(dist);
        ...
        if (s < 1) {  // sponge layer
            ...
            u(IM1,j,i) -= dt*w(IDN,j,i)*w(IVX,j,i)*pow((1.-s), 2)/sponge_tau;
            u(IM2,j,i) -= dt*w(IDN,j,i)*w(IVY,j,i)*pow((1.-s), 2)/sponge_tau;
...
void MeshBlock::UserWorkInLoop()        //called at the end of every time step
{
	...
	std::cout << "UWIL time: " << time << "\n";
        ...
        //Real vpol = uniform(gen) > polarity ? 1 : -1;       //? makes an if else statement, uniform(gen) makes random number
        Real vpol = 1;
        //if (vpol > 0) // cyclone        //basically a 50/50 chance of generating a cyclone or anticyclone? depends on dist
        //range = std::max(0., sin(max_cyclone/180.*M_PI) - sin(min_lat));
        //else { // anticyclone
            //range = std::max(0., sin(max_anticyclone/180.*M_PI) - sin(min_lat));
            //vpol *= skewness;
        //}
        //Real vlat = asin(sin(min_lat) + uniform(gen)*range);    //generates a random lat and lon in the computed range
	Real vlat = uniform(gen) * 15. + 75.;
	...
	std::cout << "lat and lon: " << vlat << " " << vlon << "\n";
        ...
        for (int i = is - NGHOST; i <= ie + NGHOST; ++i) {
            ...
            Real phi = vpol*(phi0/2)*exp(-0.5*sqr(dist)/sqr(500000));                   //sets pressure field
            Real fcor = 2.*omega*sin(lat);
            if (vpol > 0){  // cyclone
            vel = -dist*fcor/2. + dist*fcor/2.*sqrt(1. - 4.*phi/(sqr(fcor*500000)));     //cyclostrophic vel. magnitude
	    } else { // anticyclone
            vel = -phi*dist/(fcor*vrad*vrad);                                           //why is this different?
	    }
	    //Real vel = -dist*fcor/2. + dist*fcor/2.*sqrt(1. - 4.*phi/(sqr(fcor*vrad)));
        ...
        tseed = time - interval*log(uniform(gen));          //sets a random time until the next one will be generated. but in interval = 9E9 ???
    }
}
*/
void Mesh::InitUserMeshData(ParameterInput *pin)        //called at beginning of simulation to initialize global parameters
{
    // forcing parameters
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin)  //allocating and initializing local variables belonging to each MeshBlock
{
...
void MeshBlock::UserWorkBeforeOutput(ParameterInput *pin) //called right before each output file is produced
{
    for (int k = ks; k <= ke; ++k)
    for (int j = js; j <= je; ++j)
    for (int i = is; i <= ie; ++i) {
        ...
        //user_out_var(0,k,j,i) = (phydro->vf(1,k,j,i+1) - phydro->vf(1,k,j,i))/dx
        //                      - (phydro->vf(3,k,j+1,i) - phydro->vf(3,k,j,i))/dy;
        user_out_var(0,k,j,i) = (phydro->w(IVY,k,j,i+1) - phydro->w(IVY,k,j,i-1))/(2.*dx)
            - (phydro->w(IVX,k,j+1,i) - phydro->w(IVX,k,j-1,i))/(2.*dy);
        Real x1 = pcoord->x1v(i);
        Real x2 = pcoord->x2v(j);
        Real dist = sqrt(x1*x1 + x2*x2);
        Real f = 2.*omega*cos(dist/radius); 		//not approx
	//Real f = 2.*omega;
        user_out_var(1,k,j,i) = (f + user_out_var(0,k,j,i))/phydro->w(IDN,k,j,i);
    }
}
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) //sets initial conditions
{
    ...
    for (int i = is - NGHOST; i <= ie + NGHOST; ++i) {
	    ...
	    //Real fcor = 2.*omega;
            ...
            //Real phi = vpos(n,3)*(-vphi*fcor*(dist + vsize)*exp(1 - (dist/vsize)) - (sqr(vphi)/(4*vsize))*(2*dist + vsize)*exp(2-((2*dist)/vsize)));  //b=1 two term force balance
	    
            Real gam = boost::math::tgamma(2./b, (1./b)*pow(dist/vsize, b)); 
	    
	    Real phi = vpos(n,3)*-fcor*vphi*vsize*exp(1./b)*pow(b, 2./b - 1.)*gam;
            //vpos(n,3) is the polarity and thus determines the sign!
            
            if (vpos(n,3) > 0) { // cyclone
            	//vel = -dist*fcor/2. + dist*fcor/2.*sqrt(1. - 4.*phi/(sqr(fcor*vsize)));        //original
                //vel = (vphi/vsize)*dist*exp(1-(dist/vsize));  //mine v2
		vel = (vphi/vsize)*dist*exp((1./b)*(1-pow((dist/vsize), b)));
	    } else{  // anticyclone
            	vel = -(vphi/vsize)*dist*exp((1./b)*(1-pow((dist/vsize), b)));
	    }
            phydro->w(IDN,j,i) += phi;
