...
void HydroSourceTerms::Coriolis123(const Real dt,const AthenaArray<Real> *flx,
...
void HydroSourceTerms::CoriolisXYZ(const Real dt,const AthenaArray<Real> *flx,
  const AthenaArray<Real> &prim, AthenaArray<Real> &cons)
{
  MeshBlock *pmb = pmy_hydro_->pmy_block;
      ...
      for (int i=pmb->is; i<=pmb->ie; ++i) {
        if (strcmp(COORDINATE_SYSTEM,"cartesian") == 0) {
          ...
          msg << "### FATAL ERROR in HydroSourceTerms::CoriolisXYZ"
              << std::endl << "Unrecognized coordinate system" << std::endl;
          ATHENA_ERROR(msg);
        }
