...
HydroDiffusion::HydroDiffusion(Hydro *phyd, ParameterInput *pin) :
    ...
    schmidt_{pin->GetOrAddReal("problem", "schmidt", 1.0)},
...
void HydroDiffusion::CalcDiffusionFlux(const AthenaArray<Real> &prim,
  ...
  if (nu_iso > 0.0) {
    ViscousFluxIso(prim, cons, visflx);
    DiffusionFluxIso(prim, cons, visflx);
  }
  if (nu_aniso > 0.0) {
    ViscousFluxAniso(prim, cons, visflx);
    DiffusionFluxAniso(prim, cons, visflx);
  }
