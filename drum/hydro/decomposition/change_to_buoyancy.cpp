...
void Decomposition::ChangeToBuoyancy(AthenaArray<Real> &w, int kl, int ku, int jl, int ju)
{
  ...
  if (has_top_neighbor) {
    ...
    for (int k=kl; k<=ku; ++k)
      for (int j=jl; j<=ju; ++j) {
        ...
        pthermo->ConstructAtmosphere(w1, T1, P1, grav, dz/2., 2, Adiabat::reversible, 0.);
  ...
  for (int k = kl; k <= ku; ++k)
    for (int j = jl; j <= ju; ++j) {
      ...
      for (int i = is; i <= ie; ++i) {
        ...
        w(IDN,k,j,i) = (pres_(k,j,i-1) - pres_(k,j,i+1))/(pco->x1v(i+1) - pco->x1v(i-1)) 
                       - dens_(k,j,i)*grav;
      }

      // fix boundary condition
      if (pmb->pbval->block_bcs[inner_x1] != BoundaryFlag::block)
        w(IDN,k,j,is) = (pres_(k,j,is) - pres_(k,j,is+1))/(pco->x1v(is+1) - pco->x1v(is)) 
                        - sqrt(dens_(k,j,is)*dens_(k,j,is+1))*grav;

      if (pmb->pbval->block_bcs[outer_x1] != BoundaryFlag::block)
        w(IDN,k,j,ie) = (pres_(k,j,ie-1) - pres_(k,j,ie))/(pco->x1v(ie) - pco->x1v(ie-1))
  ...
  AthenaArray<Real> &wl, AthenaArray<Real> &wr, int k, int j, int il, int iu)
{
  ...
  for (int i = il; i <= iu; ++i) {
    ...
    mdpdz = (w(IPR,k,j,i-1) - w(IPR,k,j,i))/(pco->x1v(i) - pco->x1v(i-1)) ;
  ...
  for (int n = 0; n < NHYDRO; ++n)
    w1[0][n] = w(n,k,j,is);
  pthermo->ConstructAtmosphere(w1, T1, P1, grav, -dz, 2, Adiabat::reversible, 0.);
