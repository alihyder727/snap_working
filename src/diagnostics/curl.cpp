    ...
    for (int k = ks; k <= ke+1; ++k)
      for (int j = js; j <= je+1; ++j)
        for (int i = is; i <= ie+1; ++i) {
          v3f2_(k,j,i) = interp_cp4(w(IVZ,k,j-2,i), w(IVZ,k,j-1,i), w(IVZ,k,j,i), w(IVZ,k,j+1,i));
          v2f3_(k,j,i) = interp_cp4(w(IVY,k-2,j,i), w(IVY,k-1,j,i), w(IVY,k,j,i), w(IVY,k+1,j,i));
          v1f3_(k,j,i) = interp_cp4(w(IVX,k-2,j,i), w(IVX,k-1,j,i), w(IVX,k,j,i), w(IVX,k+1,j,i));
          v3f1_(k,j,i) = interp_cp4(w(IVZ,k,j,i-2), w(IVZ,k,j,i-1), w(IVZ,k,j,i), w(IVZ,k,j,i+1));
          v2f1_(k,j,i) = interp_cp4(w(IVY,k,j,i-2), w(IVY,k,j,i-1), w(IVY,k,j,i), w(IVY,k,j,i+1));
          v1f2_(k,j,i) = interp_cp4(w(IVX,k,j-2,i), w(IVX,k,j-1,i), w(IVX,k,j,i), w(IVX,k,j+1,i));
        }
  } else if (pmb->block_size.nx2 > 1) {
    for (int j = js; j <= je+1; ++j)
      for (int i = is; i <= ie+1; ++i) {
        v2f1_(ks,j,i) = interp_cp4(w(IVY,ks,j,i-2), w(IVY,ks,j,i-1), w(IVY,ks,j,i), w(IVY,ks,j,i+1));
        v1f2_(ks,j,i) = interp_cp4(w(IVX,ks,j-2,i), w(IVX,ks,j-1,i), w(IVX,ks,j,i), w(IVX,ks,j+1,i));
      }
  }
