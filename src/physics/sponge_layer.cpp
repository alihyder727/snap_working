  ...
  AthenaArray<Real> const& w, Real time, Real dt)
{
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k)
    for (int j = pmb->js; j <= pmb->je; ++j) {
      ...
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        Real scale = sqr(sin(M_PI/2.*eta));
        du(IVX,k,j,i) -= w(IDN,k,j,i)*w(IVX,k,j,i)/tau_left_*scale*dt;
        du(IVY,k,j,i) -= w(IDN,k,j,i)*w(IVY,k,j,i)/tau_left_*scale*dt;
        du(IVZ,k,j,i) -= w(IDN,k,j,i)*w(IVZ,k,j,i)/tau_left_*scale*dt;
      }
    }
  ...
  AthenaArray<Real> const& w, Real time, Real dt)
{
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k)
    for (int j = pmb->je; j <= pmb->js; --j) {
      ...
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        Real scale = sqr(sin(M_PI/2.*eta));
        du(IVX,k,j,i) -= w(IDN,k,j,i)*w(IVX,k,j,i)/tau_right_*scale*dt;
        du(IVY,k,j,i) -= w(IDN,k,j,i)*w(IVY,k,j,i)/tau_right_*scale*dt;
        du(IVZ,k,j,i) -= w(IDN,k,j,i)*w(IVZ,k,j,i)/tau_right_*scale*dt;
      }
    }
