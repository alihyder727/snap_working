  ...
  AthenaArray<Real> &du, AthenaArray<Real> const &w, Real dt)
{
  ...
  for (std::vector<MaterialPoint>::iterator q = mp.begin(); q != mp.end(); ++q) {
    ...
    interpnf(&q->v1, loc+2-f, v1.data(), xcenter_.data()+2-f, dims_.data()+2-f, 1+f);
    if (pm->f2)
      interpnf(&q->v2, loc+2-f, v2.data(), xcenter_.data()+2-f, dims_.data()+2-f, 1+f);
    else q->v2 = 0.;
    if (pm->f3)
      interpnf(&q->v3, loc+2-f, v3.data(), xcenter_.data()+2-f, dims_.data()+2-f, 1+f);
    else q->v3 = 0.;
    ...
    if (std::isnan(q->v1) || std::isnan(q->v2) || std::isnan(q->v3)) {
      ...
      std::cout << i << " " << pcoord->x1v(i) << " " << pcoord->x1v(i+1) << std::endl;
      std::cout << j << " " << pcoord->x2v(j) << " " << pcoord->x2v(j+1) << std::endl;
