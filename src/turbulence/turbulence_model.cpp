...
TurbulenceModel::TurbulenceModel(MeshBlock *pmb, ParameterInput *pin, int nvar)  :
    ...
    coarse_s_(nvar, pmb->ncc3, pmb->ncc2, pmb->ncc1,
              (pmb->pmy_mesh->multilevel ? AthenaArray<Real>::DataStatus::allocated :
               AthenaArray<Real>::DataStatus::empty)),
    coarse_r_(nvar, pmb->ncc3, pmb->ncc2, pmb->ncc1,
              (pmb->pmy_mesh->multilevel ? AthenaArray<Real>::DataStatus::allocated :
               AthenaArray<Real>::DataStatus::empty)),
...
void TurbulenceModel::calculateFluxes(AthenaArray<Real> &r, const int order) {
  ...
  // TurbulenceModel class/feature implementation is inherently coupled to Hydro class
